...
function makeTrip(overrides: Partial<Trip> = {}): Trip {
  const now = Date.now()
  return {
    id: 't1', name: '京都·大阪 家庭行', homeCurrency: 'MYR', startDate: '2026-09-02', endDate: '2026-09-06',
...
function makeBundle(overrides: Partial<ExportBundle> = {}): ExportBundle {
  return {
    ...
    homeCurrency: 'MYR',
    rows: [
      { date: '2026-09-02', type: '行程', title: '抵达关西机场', location: null, categoryName: null, amount: null, currency: null, homeAmount: null, payerName: null, note: null },
      { date: '2026-09-02', type: '账目', title: '餐饮', location: null, categoryName: '餐饮', amount: 300, currency: 'MYR', homeAmount: 300, payerName: '爸爸', note: null },
    ],
...
describe('buildJsonFile', () => {
  it('导出内容字段完整，且金额与传入数据一致', async () => {
    const file = buildJsonFile(makeBundle())
...
describe('buildCsvFile', () => {
  it('带BOM且表头与数据行数正确', async () => {
    const file = buildCsvFile(makeBundle())
  ...
  it('字段里带逗号时正确加引号转义', async () => {
    const bundle = makeBundle({
      rows: [
        { date: '2026-09-02', type: '账目', title: '餐饮, 含小费', location: null, categoryName: '餐饮', amount: 50, currency: 'MYR', homeAmount: 50, payerName: '爸爸', note: null },
      ],
    })
    const file = buildCsvFile(bundle)
...
describe('buildExcelFile', () => {
  it('生成的workbook含明细和汇总两个sheet，行数与数据一致', async () => {
    const file = buildExcelFile(makeBundle())
