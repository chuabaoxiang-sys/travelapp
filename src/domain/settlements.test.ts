...
describe('isExpenseSettled', () => {
  ...
  it('哪怕只有一条结算记录（部分结清）也算已结算', async () => {
    await db.settlements.add({
      ...
      settledDate: '2026-09-01', note: null, createdBy: 'a', expenseId: 'exp-lock-test', isPrepayment: false, deletedAt: null, createdAt: 0, updatedAt: 0,
    })
  ...
  it('聚合结算（expenseId为null）不会误判成这笔账目被结算', async () => {
    await db.settlements.add({
      ...
      settledDate: '2026-09-01', note: null, createdBy: 'a', expenseId: null, isPrepayment: false, deletedAt: null, createdAt: 0, updatedAt: 0,
    })
    expect(await isExpenseSettled('exp-lock-test')).toBe(false)
  })
})
