      ...
      <div className="max-w-[600px] mx-auto pb-20">
        {showExpense && (
          <div className="bg-[#111111] text-white px-7 pt-10 pb-7">
            <div className="text-[11px] tracking-[0.18em] text-white/50 uppercase">旅记 · 旅程结算单</div>
            ...
            <div className="mt-6 pt-5 border-t border-dashed border-white/20">
              <div className="text-[11px] tracking-[0.14em] text-white/50 uppercase">全程总花费</div>
              ...
              {!!data.expenseCategories?.length && (
                <div className="flex flex-wrap gap-x-4.5 mt-4">
                  {data.expenseCategories.map((c) => (
                    <div key={c.name} className="flex items-baseline gap-1.5 text-[12px] text-white/60 py-1">
                      <span>{c.name}</span>
        ...
        {!showExpense && (
          <div className="px-7 pt-10 pb-7 border-b border-[#E6E6E6]">
            <div className="text-[11px] tracking-[0.18em] text-[#7A7A7A] uppercase">旅记 · 旅程结算单</div>
            <div className="mt-2 font-semibold text-[22px] tracking-tight text-wrap-balance">{data.name}</div>
            {data.startDate && data.endDate && (
        ...
        {showItinerary && (
          <div className="px-7 pt-7">
            <div className="text-[11px] tracking-[0.12em] text-[#7A7A7A] uppercase mb-1">行程活动记录</div>
            {data.days!.map((day, i) => (
