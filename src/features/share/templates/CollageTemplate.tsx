  ...
  return (
    <div className="min-h-screen bg-[#E3E9E3] text-[#17251E]" style={{ fontFamily: '"Noto Sans SC", -apple-system, system-ui, sans-serif' }}>
      <div className="max-w-[640px] mx-auto">
        ...
        >
          <div className="text-[10.5px] tracking-[0.28em] text-[#EDF2EA]/60">{t('shareTemplates.collage.eyebrow')}</div>
              ...
              <div key={i} className="relative bg-[#F7F9F5] rounded px-3.5 pt-3.5 pb-3 mb-3.5 shadow-[0_2px_0_rgba(23,37,30,0.08)]">
                ...
                <div className="flex items-baseline gap-2">
                  <div className="font-bold text-[17px]" style={{ fontFamily: '"Noto Serif SC", serif' }}>
                    {t('shareTemplates.collage.day', { day: i + 1 })}
                  </div>
                  <div className="text-[12px] text-[#5C7266] tabular">{formatDotDate(day.dayDate)}</div>
                  <div className="text-[11px] text-[#5C7266]">{formatDow(day.dayDate, i18n.language)}</div>
                </div>
        ...
        {showExpense && (
          <div className="bg-[#1E3A2F] text-[#EDF2EA] px-5 pt-4 pb-4.5">
            <div className="flex justify-between items-baseline">
              <div className="text-[11px] tracking-[0.13em] text-[#EDF2EA]/60">{t('shareTemplates.common.totalSpentCasual')}</div>
              <div className="text-[22px] font-semibold tabular text-[#F2C230]">{formatMoney(data.expenseTotal!)}</div>
            </div>
            {!!data.expenseCategories?.length && (
              <div className="mt-2.5 flex flex-wrap gap-1.5">
                {data.expenseCategories.map((c) => (
                  <div key={c.id} className="bg-[#EDF2EA]/12 rounded px-2.5 py-1 text-[11px]">
                    {categoryLabel(c, t)} <span className="tabular font-medium text-[#C9E4C0]">{formatMoney(c.amount)}</span>
                  </div>
                ))}
              </div>
            )}
            <div className="mt-3.5 text-[10.5px] tracking-[0.1em] text-[#EDF2EA]/50">{t('shareTemplates.common.footerShort', { brand: t('common.appName') })}</div>
          </div>
        )}
      </div>
    </div>
